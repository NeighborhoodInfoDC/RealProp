/**************************************************************************
 ...
 Modifications:
  ...
  07/09/13 LH  Moved to Realprop library from HsngMon
  07/14/14 PAT Added code to assign values to macro vars g_sales_start_yr,
               g_sales_end_yr, and g_sales_mid_yr when start and end dates
               are set manually through sales_start_dt= and sales_end_dt=.
**************************************************************************/

/** Macro Init_macro_vars - Start Definition **/

%macro Init_macro_vars( rpt_yr=, rpt_qtr=, sales_start_dt=, sales_end_dt=, sales_qtr_offset=-3 );
  ...
  %if &sales_start_dt ~= %then %do;
    ...
    %let g_sales_start_yr = %sysfunc( year( &sales_start_dt ) );
    %let g_sales_end_yr = %sysfunc( year( &sales_end_dt ) );
    %let g_sales_mid_yr = %eval( &g_sales_start_yr + %sysfunc( int( %eval( ( &g_sales_end_yr - &g_sales_start_yr ) / 2 ) ) ) );
  %end;
  %else %do;
    data _null_;
  ...
  %let g_path = &_dcdata_l_path\realprop\Prog\quarterly\&rpt_yr-&rpt_qtr;
  ...
  %let g_table_wbk = DC Quarterly Sales Data &g_rpt_title tables.xls;
  %let g_fig_wbk = DC Quarterly Sales Data &g_rpt_title figures.xls;
