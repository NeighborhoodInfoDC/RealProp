/**************************************************************************
 Program:  Read_CAMACommPt_macro.sas
 Library:  RealProp
 ...
 Description:  Autocall macro for reading and processing CAMACommPt
...
/** Macro Read_CAMACommPt_macro - Start Definition **/

%macro Read_CAMACommPt_macro( 
  inpath= &_dcdata_r_path\RealProp\Raw,    /* Location of input file */
  infile= CAMACommPt,                             /* Name of input file */
  ...
  format=                                      /* Additional formats to be applied */
    deed_date CAMACommPt_SALEDATE saledate mmddyy10.
    Usecode $Usecode.
    Hstd_code $homestd.
    ...
    Saletype $Saletyp.,
  freqvars=
    CAMACommPt_SALEDATE qdrntname 
    saletype usecode 
    

  );
  
  %let mname=Read_CAMACommPt_macro;
  /**%syslput mname = &mname;**/
  ...
  ** Convert file from DBF or MDB format **;
  ...
  %if &intype = DBF %then %do;
    libname inf dbdbf "&inpath" ver=4 CAMACommPt=&infile;
    %let dsname = CAMACommPt;
  %end;
  %else %if &intype = MDB %then %do;
    %if &intable = %then %do;
      ...
      %goto exit;
    %end;
    libname inf dbaccess "&inpath\&infile..mdb" CAMACommPt="&intable";
    %let dsname = CAMACommPt;
  %end;
  ...
  ** Get file extraction date **;
  ...
  %let outfile = CAMACommPt_&fileyear._%sysfunc( putn( &filemonth, z2. ) )&outfilesuf;
  ...
  %if &D31MAR2013 > &filedate >= &D24MAR2005 %then %do;
    ...
    %let format=
      deed_date CAMACommPt_SALEDATE saledate lastpaydt mmddyy10.
      Usecode $Usecode.
      ;
  ...
  ** Create final CAMACommPt extract file **;
  ...
  **rsubmit;
  ...
  data &outfile (label="DC real property parcels update file, extract date &filedate_fmt");
    ...
    set CAMACommPt (drop=SALEDATE);

    retain CAMACommPt_SALEDATE &filedate;
    ...
    ** Data set labels **;
    ...
    %include "&_dcdata_r_path\RealProp\Prog\Updates\Label_CAMAcomm.sas";
...
options spool;
  ...
  data RealPr_r.CAMACommPt;
	keep	AC	AYB	BATHRM	BEDRM	BLDG_NUM	CNDTN	CNDTN_D	EXE_CODE	EXTWALL	EXTWALL_D	
			...
			STRUCT	STRUCT_D	STYLE	STYLE_D	UNITNUMBER	USECODE	X_COORD	Y_COORD	YR_RMDL;
  ...
  **endrsubmit;
  ...
  %else %do;
    ...
    %note_mput( macro=&mname, msg=Previous CAMACommPt file not specified (PREV_FILE=). )
    %note_mput( macro=&mname, msg=Comparison with previous CAMACommPt will not be performed. )
