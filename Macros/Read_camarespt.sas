/**************************************************************************
 ...
 Modifications: 04/15/14 LH - Removed unneeded code from ownerpt macro. /
...
%macro Read_CAMAResPt( 
  ...
  format=                                      /* Additional formats to be applied */
    ...
    ,
  freqvars= usecode cama_proptype
    
  );
  
  %let mname=Read_CAMAResPt_macro;
  /**%syslput mname = &mname;**/
  ...
  ** Convert file from DBF or MDB format **;
  ...
  %if &intype = DBF %then %do;
    libname inf dbdbf "&inpath" ver=4 CAMAResPt=&infile;
    %let dsname = CAMAResPt;
  %end;
  %else %if &intype = MDB %then %do;
    %if &intable = %then %do;
      ...
      %goto exit;
    %end;
    libname inf dbaccess "&inpath\&infile..mdb" CAMAResPt="&intable";
    %let dsname = CAMAResPt;
  %end;
  ...
  %let outfile = CAMAResPt_&fileyear._%sysfunc( putn( &filemonth, z2. ) )&outfilesuf;
 ...
 ** Create final CAMAResPt extract file **;
  ...
  data &outfile (label="DC real property CAMA ResPt update file, extract date &filemonth.-&fileyear.");
    ...
    ** Data set labels **;
    ...
    %include "&_dcdata_r_path\RealProp\Prog\Updates\Label_CAMA.sas";
  ...
  **rsubmit;
  ...
  data RealPr_r.camarespt;
	keep 	AC AYB	BATHRM	BEDRM	BLDG_NUM	CNDTN	CNDTN_D	EXE_CODE	EXTWALL	EXTWALL_D	
			...
			KITCHENS	LANDAREA LIVING	OWNERNAME	OWNNAME2	PREMISEADD	
			PRICE	QUALIFIED	ROOF	ROOF_D	ROOMS	SALE_NUM	SALEDATE	SSL	STORIES	
			STRUCT	STRUCT_D	STYLE	STYLE_D	UNITNUMBER	USECODE	X_COORD	Y_COORD	YR_RMDL cama_proptype;
